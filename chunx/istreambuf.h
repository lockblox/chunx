...
InputStreamBuffer<InputIterator>::InputStreamBuffer(InputIterator begin,
  ...
  auto read_end =
      const_cast<char*>(&read_buffer_.front()) + read_buffer_.size();
...
std::streambuf::int_type InputStreamBuffer<InputIterator>::underflow() {
  ...
  while (write_iter != read_buffer_.cend()) {
    if (read_it_ == it_->cend()) {
      // Load another block if necessary and one is available
      if (++it_ == end_) {
        break;
      } else {
        read_it_ = it_->cbegin();
...
template <typename InputIterator>
std::streambuf::pos_type InputStreamBuffer<InputIterator>::seekoff(
    std::streambuf::off_type off, std::ios_base::seekdir dir,
    std::ios_base::openmode which) {
  if (begin_ == end_) {
    throw std::out_of_range("seek in empty file");
  }
  if (dir == std::ios_base::cur) {  // just keep iterators where they are
    ...
    auto node = it_;
    while (node != end_) {  // advance it_ to penultimate node
...
template <typename InputIterator>
std::streambuf::pos_type InputStreamBuffer<InputIterator>::seekpos(
